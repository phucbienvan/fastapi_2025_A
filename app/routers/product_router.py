...
@router.delete("/products/{product_id}/force", tags=["products"], description="Delete a product by id", response_model=DataResponse[ProductSchema])
...
@router.put("/products/{product_id}", tags=["products"], description="Update a product", response_model=DataResponse[ProductSchema])
...
@router.delete("/products/{product_id}", tags=["products"], description="Soft delete a product", response_model=DataResponse[ProductSchema])
def soft_delete_product(product_id: int, db: Session = Depends(get_db)):
    ...
    db.commit()
    return DataResponse.custom_response(code="200", message="Product deleted successfully (soft)", data=None)
