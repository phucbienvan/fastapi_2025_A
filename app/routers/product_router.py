...
def delete_product(product_id: int, db: Session = Depends(get_db)):
    ...
    db.commit()
    return DataResponse.custom_response(code="200", message="Soft deleted product by id", data=None)
...
async def update_product(product_id: int, data: CreateProductSchema, db: Session = Depends(get_db)):
    product = db.query(Product).filter(Product.id == product_id, Product.is_deleted == False).first()
    if not product:
        return DataResponse.custom_response(code="404", message="Product not found", data=None)
